...
export async function findWalletByName(name: string) {
  if (!name) {
    throw new BadRequestException('Tên ví không hợp lệ');
  }
  const [get_wallet_by_name] = await db
    .select({
    ...
    .where(
      and(eq(wallets.walletName, name), eq(wallets.walletIsDeleted, false)),
    )
    .limit(1);
  if (!get_wallet_by_name) {
