...
export async function checkBalance(userId: string, requiredAmount: number) {
  ...
  if (currentBalance <= 0) {
    throw new BadRequestException('Bạn không đủ tiền trong ví');
  }
  if (currentBalance < requiredAmount) {
    throw new BadRequestException(
      `Số dư không đủ. Hiện tại: ${currentBalance.toLocaleString('vi-VN')} VNĐ, cần: ${requiredAmount.toLocaleString('vi-VN')} VNĐ`,
    );
  }
