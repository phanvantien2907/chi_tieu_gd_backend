...
import { settlements, walletMembers, wallets, users } from 'src/database/schema';
...
export class SettlementsService {
  async findAll() {
    const settlements_with_payer = await db
      .select({
      ...
      .innerJoin(
        walletMembers,
        eq(settlements.settlementPayerId, walletMembers.memberId),
      )
    ...
    if (settlements_with_payer.length === 0) {
      throw new NotFoundException('Không có khoản nợ nào!');
    }
    return {
      status: 200,
  ...
  async findSettlementsByID(id: string) {
    if (!id || !isUuid(id)) {
      throw new NotFoundException('ID khoản nợ không hợp lệ!');
    }
    const settlements_with_payer = await db
      .select({
        settlementId: settlements.settlementId,
      ...
      .innerJoin(users, eq(walletMembers.memberUserId, users.userId))
      .where(
        and(eq(settlements.isPaid, false), eq(settlements.settlementId, id)),
      );
  ...
  async getStats() {
    ...
    const creditorStats = await db
      ...
      .innerJoin(
        walletMembers,
        eq(settlements.settlementReceiverId, walletMembers.memberId),
      )
      .innerJoin(users, eq(walletMembers.memberUserId, users.userId))
      .where(eq(settlements.isPaid, false));
    ...
    creditorStats.forEach((item) => {
      ...
      if (creditorMap.has(item.receiverName)) {
        creditorMap.set(
          item.receiverName,
          creditorMap.get(item.receiverName) + amount,
        );
      } else {
    ...
    const walletBreakdown = Array.from(walletMap.entries()).map(
      ([name, stats]) => ({
        walletName: name,
        ...
        overview: {
          ...
          paymentRate:
            allSettlements.length > 0
              ? Math.round((paidCount / allSettlements.length) * 100)
              : 0,
        },
        topDebtors,
        topCreditors,
        alerts: {
          highRiskDebtors: debtorStats.filter(
            (s) => parseFloat(s.settlementAmount) > 100000,
          ).length,
          totalUnpaidCount: unpaidCount,
          averageDebtAmount:
            unpaidCount > 0
              ? Math.round(totalUnpaidAmount / unpaidCount).toString()
              : '0',
        },
        walletBreakdown,
      },
