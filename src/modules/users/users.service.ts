...
export class UsersService {
  async create(createUserDto: CreateUserDto) {
    ...
    if (!create_user) {
      throw new NotFoundException('Tạo người dùng không thành công');
    }
    return {
  ...
  async findAll() {
    const find_list_user = await db
      ...
      .from(users)
      .where(eq(users.userIsDeleted, false))
    ...
    return {
      ...
      msg: `Lấy danh sách ${find_list_user.length} người dùng thành công`,
      data: find_list_user,
    };
  }
  ...
  async update(id: string, updateUserDto: UpdateUserDto) {
    ...
    const update_user = await db
      .update(users)
      .set({
        ...(updateUserDto.userFullName && {
          userFullName: updateUserDto.userFullName,
        }),
        ...(updateUserDto.userHashedPassword && {
          userHashedPassword: await hashPassword(
            updateUserDto.userHashedPassword,
          ),
        }),
        ...(updateUserDto.userEmail && { userEmail: updateUserDto.userEmail }),
        ...(updateUserDto.userAvatarUrl && {
          userAvatarUrl: updateUserDto.userAvatarUrl,
        }),
    ...
    if (!update_user) {
      throw new NotFoundException('Cập nhật thông tin không thành công');
    }
    return {
      status: HttpStatus.OK,
  ...
  async remove(id: string) {
    ...
    const delete_user = await db
      ...
      .where(eq(users.userId, find_user_by_id.userId));
    if (!delete_user) {
      throw new NotFoundException('Xóa người dùng không thành công');
    }
    return {
  ...
  async findUserById(id: string) {
    ...
    const [user] = await db
      ...
      .from(users)
      .where(and(eq(users.userId, id), eq(users.userIsDeleted, false)))
      .limit(1);
    if (!user) {
