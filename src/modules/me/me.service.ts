...
export class MeService {
  async me(userId: string) {
    ...
    return {
      ...
      msg: 'Lấy thông tin trang cá nhân thành công',
      data: user,
    };
  }
  ...
  async update(@Body() UpdateMeDto: UpdateMeDto, userId: string) {
    ...
    const update_user = await db
      ...
      .set({
        ...
        ...(UpdateMeDto.user_full_name && {
          userFullName: UpdateMeDto.user_full_name,
        }),
        ...(UpdateMeDto.user_avatar_url && {
          userAvatarUrl: UpdateMeDto.user_avatar_url,
        }),
  ...
  async updatePassword(
    ...
    if (!update_password) {
      throw new BadRequestException('Cập nhật mật khẩu không thành công');
    }
  ...
  async deleteAccount(userId: string) {
    ...
    const delete_user = await db
      ...
      .where(eq(users.userId, find_user.userId));
    if (!delete_user) {
      throw new BadRequestException('Xoá tài khoản không thành công');
    }
