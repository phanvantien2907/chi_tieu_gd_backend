...
import { expenses, settlements, walletMembers, users } from 'src/database/schema';
...
export class ExpensesService {
  async findAll() {
    const get_all_expenses = await db
      .select({
      ...
      .innerJoin(
        walletMembers,
        eq(expenses.expensePayerId, walletMembers.memberId),
      )
      .where(eq(expenses.expenseIsDeleted, false))
      .orderBy(expenses.expenseDate);
    if (get_all_expenses.length === 0) {
      throw new NotFoundException('Không tìm thấy chi tiêu nào!');
    }
    const result: any[] = [];
  ...
  async remove(id: string) {
    ...
    if (!delete_expense) {
      throw new BadRequestException('Xoá chi tiêu không thành công!');
    }
  ...
  async findExpenseById(id: string) {
    if (!id || !isUuid(id)) {
      throw new NotFoundException('ID chi tiêu không hợp lệ!');
    }
    const find_expense_by_id = await db
      .select({
        expenseId: expenses.expenseId,
        expenseWalletId: expenses.expenseWalletId,
        expenseDescription: expenses.expenseDescription,
      ...
      .where(
        and(eq(expenses.expenseId, id), eq(expenses.expenseIsDeleted, false)),
      )
      .limit(1);
    if (!find_expense_by_id.length) {
      throw new NotFoundException('Không tìm thấy chi tiêu nào!');
    }
