...
export const users = pgTable(
  'users',
  {
    ...
    userCreatedAt: timestamp('user_created_at', {
      withTimezone: true,
      mode: 'string',
    })
      .defaultNow()
  ...
  (table) => [unique('users_user_email_key').on(table.userEmail)],
);
...
export const wallets = pgTable(
  'wallets',
  {
    ...
    walletCurrency: varchar('wallet_currency', { length: 3 })
      .default('VND')
      .notNull(),
  ...
  (table) => [
    ...
    uniqueIndex('wallets_name_created_by_unique_idx')
      .on(table.walletName, table.walletCreatedBy)
      .where(sql`wallet_is_deleted = false`),
  ],
);
...
export const walletMembers = pgTable(
  'wallet_members',
  {
    ...
    memberJoinedAt: timestamp('member_joined_at', {
      withTimezone: true,
      mode: 'string',
    })
      .defaultNow()
  ...
  (table) => [
    ...
    unique('wallet_members_member_wallet_id_member_user_id_key').on(
      table.memberWalletId,
      table.memberUserId,
    ),
  ],
);
...
export const categories = pgTable(
  'categories',
  {
    ...
    categoryCreatedAt: timestamp('category_created_at', {
      withTimezone: true,
      mode: 'string',
    })
      .defaultNow()
  ...
  (table) => [
    ...
    unique('categories_category_wallet_id_category_name_key').on(
      table.categoryWalletId,
      table.categoryName,
    ),
  ],
);
...
export const expenseSplits = pgTable(
  ...
  (table) => [
    ...
    unique('expense_splits_split_expense_id_split_user_id_key').on(
      table.splitExpenseId,
      table.splitUserId,
    ),
    check(
      'expense_splits_split_amount_check',
      sql`split_amount > (0)::numeric`,
    ),
  ],
);
...
export const expenses = pgTable(
  'expenses',
  {
    ...
    expenseDate: timestamp('expense_date', {
      ...
      .defaultNow()
      .notNull(),
    expenseCreatedAt: timestamp('expense_created_at', {
      withTimezone: true,
      mode: 'string',
    })
      .defaultNow()
      .notNull(),
    expenseUpdatedAt: timestamp('expense_updated_at', {
      withTimezone: true,
      mode: 'string',
    })
      .defaultNow()
...
export const walletTransactions = pgTable(
  'wallet_transactions',
  {
    ...
    transactionCreatedAt: timestamp('transaction_created_at', {
      withTimezone: true,
      mode: 'string',
    })
      .defaultNow()
      .notNull(),
    transactionUpdatedAt: timestamp('transaction_updated_at', {
      withTimezone: true,
      mode: 'string',
    })
      .defaultNow()
  ...
  (table) => [
    index('idx_wallet_transactions_user_id').using(
      'btree',
      table.transactionUserId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_wallet_transactions_wallet_id').using(
      'btree',
      table.transactionWalletId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const walletBalances = pgTable(
  'wallet_balances',
  {
    ...
    balanceAmount: numeric('balance_amount', { precision: 15, scale: 2 })
      .default('0')
      .notNull(),
  ...
  (table) => [
    index('idx_wallet_balances_wallet_user').using(
      'btree',
      table.balanceWalletId.asc().nullsLast().op('uuid_ops'),
      table.balanceUserId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
    ...
    unique('wallet_balances_balance_wallet_id_balance_user_id_key').on(
      table.balanceWalletId,
      table.balanceUserId,
    ),
  ],
);
...
export const settlements = pgTable(
  ...
  (table) => [
    ...
    check(
      'settlements_check',
      sql`settlement_payer_id <> settlement_receiver_id`,
    ),
    check(
      'settlements_settlement_amount_check',
      sql`settlement_amount > (0)::numeric`,
    ),
  ],
);
...
export const refreshTokens = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.refreshId, table.userId],
